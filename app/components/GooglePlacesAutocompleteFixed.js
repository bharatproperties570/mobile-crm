  ...
  const _request = (text) => {
    ...
    if (supportedPlatform() && text && text.length >= minLength) {
      ...
      request.onreadystatechange = () => {
        ...
        setListLoaderDisplayed(false);
        console.log('GooglePlaces Response Status:', request.status);

        if (request.status === 200) {
          const responseJSON = JSON.parse(request.responseText);
          console.log('GooglePlaces Response Data:', responseJSON);
      ...
      console.log('GooglePlaces Request URL:', request._url || request.responseURL || 'Check Network Tab');
